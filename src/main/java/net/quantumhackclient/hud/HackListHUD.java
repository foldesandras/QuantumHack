...
import net.quantumhackclient.other_features.HackListOtf;
import net.quantumhackclient.other_features.HackListOtf.Mode;
import net.quantumhackclient.other_features.HackListOtf.Position;
...
public final class HackListHUD implements UpdateListener
{
	...
	public void render(DrawContext context, float partialTicks)
	{
		...
		if(otf.getPosition() == Position.LEFT
			&& QuantumHackClient.INSTANCE.getOtfs().wurstLogoOtf.isVisible())
			posY = 22;
		else
			posY = 2;
		...
		{
			float[] acColor = QuantumHackClient.INSTANCE.getGui().getAcColor();
	...
	private void drawString(DrawContext context, String s)
	{
		...
		{
			int screenWidth = QuantumHackClient.MC.getWindow().getScaledWidth();
