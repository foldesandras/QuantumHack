...
import net.quantumhackclient.clickgui.SettingsWindow;
...
{
	private final MinecraftClient MC = QuantumHackClient.MC;
	private final ClickGui GUI = QuantumHackClient.INSTANCE.getGui();
		...
		int y1, int y2, boolean hHack)
	{
		...
		if(feature.isEnabled())
			...
			// else
			RenderSystem.setShaderColor(0, 0, 1,
		...
		int y1, int y2, boolean hSettings)
	{
		...
		}else
		{
			ya1 = y1 + 3.5F;
			ya2 = y2 - 3;
			RenderSystem.setShaderColor(0, 0, hSettings ? 1 : 0.85F, 1);
		}
