 ...
 */
package net.quantumhackclient.clickgui;
...
import net.quantumhackclient.clickgui.components.FeatureButton;
...
public final class ClickGui
{
	...
	public void init()
	{
		...
		features.addAll(QUANTUM_HACK_CLIENT.getHax().getAllHax());
		features.addAll(QUANTUM_HACK_CLIENT.getCmds().getAllCmds());
		features.addAll(QUANTUM_HACK_CLIENT.getOtfs().getAllOtfs());
		...
		windows.add(QuantumHackClient.INSTANCE.getHax().radarHack.getWindow());
	...
	{
		ClickGuiHack clickGui = QUANTUM_HACK_CLIENT.getHax().clickGuiHack;
		...
		int x2, int y2, boolean hovering, boolean minimized)
	{
		...
		if(minimized)
		{
			ya1 = y1 + 3;
			ya2 = y2 - 2.5F;
			RenderSystem.setShaderColor(0, 0, hovering ? 1 : 0.85F, 1);
		...
		int x2, int y2, boolean hovering, boolean pinned)
	{
		...
		if(pinned)
		{
			...
			// knob
			RenderSystem.setShaderColor(0, 0, h, 0.5F);
			bufferBuilder.begin(VertexFormat.DrawMode.QUADS,
				VertexFormats.POSITION);
		...
		}else
		{
			...
			// knob
			RenderSystem.setShaderColor(0, 0, h, 1);
			bufferBuilder.begin(VertexFormat.DrawMode.QUADS,
				VertexFormats.POSITION);
			bufferBuilder.vertex(matrix, xk1, yk1, 0).next();
			bufferBuilder.vertex(matrix, xk2, yk2, 0).next();
