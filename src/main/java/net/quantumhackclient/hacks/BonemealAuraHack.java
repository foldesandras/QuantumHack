...
import net.quantumhackclient.events.HandleInputListener;
...
import net.quantumhackclient.util.InteractionSimulator;
import net.quantumhackclient.util.InventoryUtils;
...
public final class BonemealAuraHack extends Hack implements HandleInputListener
{
	...
	public void onHandleInput()
	{
		...
		// wait for AutoFarm
		if(QUANTUM_HACK.getHax().autoFarmHack.isBusy())
			return;
	...
	private boolean isCorrectBlock(BlockPos pos)
	{
		...
		if(block instanceof SaplingBlock sapling
			&& sapling.isFertilizable(world, pos, state, true))
			return saplings.isChecked();
		
		if(block instanceof CropBlock crop
			&& crop.isFertilizable(world, pos, state, true))
			return crops.isChecked();
		
		if(block instanceof StemBlock stem
			&& stem.isFertilizable(world, pos, state, true))
			return stems.isChecked();
		
		if(block instanceof CocoaBlock cocoaBlock
			&& cocoaBlock.isFertilizable(world, pos, state, true))
