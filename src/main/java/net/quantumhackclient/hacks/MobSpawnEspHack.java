...
import net.quantumhackclient.events.PacketInputListener;
...
import net.quantumhackclient.settings.ChunkAreaSetting;
import net.quantumhackclient.settings.ChunkAreaSetting.ChunkArea;
...
import net.quantumhackclient.util.RegionPos;
import net.quantumhackclient.util.RenderUtils;
import net.quantumhackclient.util.chunk.ChunkSearcher;
import net.quantumhackclient.util.chunk.ChunkSearcher.Result;
import net.quantumhackclient.util.chunk.ChunkVertexBufferCoordinator;
	...
	implements UpdateListener, RenderListener
{
	...
	private boolean isSpawnable(BlockPos pos, BlockState state)
	{
		...
		// EntityType<?>)
		Location location = SpawnRestriction.getLocation(EntityType.CREEPER);
		if(!SpawnHelper.canSpawn(location, MC.world, pos, EntityType.CREEPER))
			return false;
			
		// Check for hitbox collisions
		// (using a creeper because it's shorter than a zombie)
		if(!MC.world.isSpaceEmpty(EntityType.CREEPER.createSimpleBoundingBox(
		...
		RegionPos region)
	{
		...
		bufferBuilder.vertex(x1, y, z1).color(color).next();
		bufferBuilder.vertex(x2, y, z2).color(color).next();
		bufferBuilder.vertex(x2, y, z1).color(color).next();
		bufferBuilder.vertex(x1, y, z2).color(color).next();
	}
