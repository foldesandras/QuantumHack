 ...
 */
package net.quantumhackclient.navigator;
...
import net.quantumhackclient.clickgui.Window;
...
public final class NavigatorFeatureScreen extends NavigatorScreen
{
	...
	protected void onResize()
	{
		...
		if(!possibleKeybinds.isEmpty())
		{
			...
			for(Keybind keybind : QuantumHackClient.INSTANCE.getKeybinds()
				.getAllKeybinds())
			{
	...
	{
		// popups
		if(QuantumHackClient.INSTANCE.getGui().handleNavigatorPopupClick(x, y,
			button))
			return;
		...
		// component settings
		QuantumHackClient.INSTANCE.getGui().handleNavigatorMouseClick(
	...
	{
		QuantumHackClient.INSTANCE.getGui().handleMouseRelease(x, y, button);
	}
	...
	{
		window.close();
		QuantumHackClient.INSTANCE.getGui().handleMouseClick(Integer.MIN_VALUE,
			Integer.MIN_VALUE, 0);
	}
